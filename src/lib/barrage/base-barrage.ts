 ...
 * 构造函数参数对象
 */
export interface BaseBarrageOptions {
  // 弹幕的唯一标识
  id: string;
  // 弹幕的出现时间（毫秒为单位）
  time: number;
...
export default abstract class BaseBarrage {
  ...
  id: string;
  // 弹幕的类型
  abstract readonly barrageType: BarrageType;
  // 弹幕的出现时间（毫秒为单位）
  time: number;
  // 弹幕的内容
  text: string;
  ...
  initBarrage() {
    ...
    sectionObjects.forEach((sectionObject) => {
      // 判断是文本片段还是图片片段
      if (
        sectionObject.type === 'image' &&
        (barrageImage = this.br.barrageImages?.find(
          (bi) => `[${bi.id}]` === sectionObject.value,
        ))
      ) {
        totalWidth += barrageImage.width;
        maxHeight =
          maxHeight < barrageImage.height ? barrageImage.height : maxHeight;

        // 构建图片片段
        sections.push(
          new ImageSection({
            ...barrageImage,
            leftOffset: Utils.Math.sum(
        ...
        const textWidth =
          this.br.ctx?.measureText(sectionObject.value).width || 0;
  ...
  render(ctx: CanvasRenderingContext2D | OffscreenCanvasRenderingContext2D) {
    ...
    // 尝试获取弹幕的缓存
    const imageBitmap = this.br.preRenderOptimizer.getImageBitmap(this);
    // 如果有缓存的 ImageBitmap 的话，直接使用 ImageBitmap 进行渲染即可
    if (imageBitmap) {
      ctx.drawImage(imageBitmap, this.left, this.top, this.width, this.height);
    } else {
      // 没有缓存的话，进行常规的渲染
      // 绘制弹幕内容
      // 设置文字相关的属性
 ...
 * 描述弹幕内容中的文本片段
 */
export interface TextSectionOptions {
  // 文本片段的内容
  text: string;
  // 这段文本的宽高
  width: number;
